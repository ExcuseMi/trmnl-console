...
```bash
cp pusher.example.yml pusher.yml
$EDITOR pusher.yml   # set webhook_url and at least one job
...
```bash
trmnl-console-pusher --config pusher.yml --once
```
...
## Pushing to TRMNL directly, or to your own backend

`webhook_url` can be either TRMNL's own webhook endpoint
(`https://usetrmnl.com/api/custom_plugins/...`, found on the plugin instance's settings page)
or a self-hosted [`trmnl-console-backend`](../backend/) URL (`https://your-host/<id>`) paired
with a **polling**-strategy plugin - see [`../backend/README.md`](../backend/README.md) for
why you'd want that (mainly: no webhook rate/size limit, since the backend isn't rate-limited
and TRMNL pulls from it on its own schedule). No extra config either way - it's just a URL:

```yaml
webhook_url: "https://your-host/<id>"
```
...
# webhook endpoint or a trmnl-console-backend URL - see above.
...
are specific to TRMNL's own webhook endpoint; pushing to a
[`trmnl-console-backend`](../backend/) instead isn't subject to either, since it just stores
whatever it's given and TRMNL pulls from it separately. If you're pushing to TRMNL directly,
keep `interval_seconds` at 300 or higher unless you're on TRMNL+, and watch the logs: a job
that's dropping size variants or still over budget after dropping down to one logs a warning
each tick.
...
The pusher needs to reach whichever tmux/Zellij *server* your sessions actually live on. If
...
```bash
cd cli-client
cargo build --release --no-default-features --bin trmnl-console-pusher
./target/release/trmnl-console-pusher --config /path/to/pusher.yml
```

`--no-default-features` skips the CLI's `preview` feature (and its dependency on the
`trmnl-framework` git submodule), which the pusher doesn't use.

## CLI flags

```
trmnl-console-pusher [--config <PATH>] [--once]
```

- `--config`/`-c` (or the `TRMNL_CONSOLE_PUSHER_CONFIG` env var, which the Docker image sets
  to `/etc/trmnl-console/pusher.yml` by default): path to the YAML config.
