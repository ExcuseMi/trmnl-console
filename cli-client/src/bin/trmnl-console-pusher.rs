//! `trmnl-console-pusher`: runs one or more scheduled capture jobs and pushes their
//! output to a TRMNL webhook-style endpoint - TRMNL's own, or a self-hosted
//! `trmnl-console-backend` - using the same TRMNL webhook-style payload (`merge_variables`,
//! see <https://docs.trmnl.com/go/private-plugins/webhooks>) as the `trmnl-console` CLI's
//! `--url` mode.
//!
...
//! actually given at render time. See `pusher/README.md` for job config examples,
...
#[command(
    version,
    about = "Runs scheduled trmnl-console capture jobs and pushes them to a TRMNL webhook-style endpoint."
)]
struct CliArgs {
    /// Path to the pusher job config file (YAML). See pusher/README.md for the format.
    #[arg(
        short,
        long,
        env = "TRMNL_CONSOLE_PUSHER_CONFIG",
        default_value = "pusher.yml"
    )]
    config: PathBuf,
...
struct JobConfig {
    ...
    /// pusher/README.md for that case).
    command: String,
...
async fn main() -> ExitCode {
    ...
    let config = match load_config(&args.config) {
        ...
        Err(err) => {
            eprintln!(
                "trmnl-console-pusher: failed to load config {}: {err}",
                args.config.display()
            );
    ...
    if config.jobs.is_empty() {
        eprintln!("trmnl-console-pusher: config has no jobs, nothing to do");
        return ExitCode::from(91);
    }
    ...
    for job in config.jobs {
        let Some(webhook_url) = job
            ...
            eprintln!(
                "trmnl-console-pusher: job '{}' has no webhook_url and no top-level default is set, skipping",
                job.name
            );
            continue;
        };
        if job.sizes.is_empty() {
            eprintln!(
                "trmnl-console-pusher: job '{}' has an empty sizes list, skipping",
                job.name
            );
            continue;
        }
    ...
    if jobs.is_empty() {
        eprintln!("trmnl-console-pusher: no runnable jobs after validation, exiting");
        return ExitCode::from(91);
    }
    ...
    eprintln!("trmnl-console-pusher: shutting down...");
...
async fn run_job_once(job: &JobConfig, webhook_url: &str) {
    match run_job(job, webhook_url).await {
        Ok(()) => println!("trmnl-console-pusher: job '{}' sent successfully", job.name),
        Err(err) => eprintln!("trmnl-console-pusher: job '{}' failed: {err}", job.name),
    }
}
...
fn trim_to_budget(
    ...
    loop {
        ...
        if size <= max_bytes || variants.len() <= 1 {
            if size > max_bytes {
                eprintln!(
                    "trmnl-console-pusher: job '{job_name}': payload is {size} bytes (budget {max_bytes}) even with only one size variant left; sending anyway",
                );
            }
            return variants;
        }
        ...
        eprintln!(
            "trmnl-console-pusher: job '{job_name}': payload was {size} bytes, over budget {max_bytes}; dropped size variant '{}'",
            dropped.id
        );
    }
}
